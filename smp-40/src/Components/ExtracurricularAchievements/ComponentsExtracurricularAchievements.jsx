// code untuk halaman utama (beranda) dan halaman achievement sebagai pembungkus dari card
import { Link } from "react-router-dom";
import FallbackAchievements from "../Data/DataExtracurricularAchievements";
import { normalizeData } from "../../utils/helper";

// fallback sudah normalisasi otomatis
const FallbackAchievementsData = normalizeData(FallbackAchievements);


const ComponentsExtracurricularAchievements = ({
  extracurricularAchievementsList,
  maxItems = 20,
  layout = "grid",
}) => {
  const source =
    Array.isArray(extracurricularAchievementsList) &&
    extracurricularAchievementsList.length
      ? extracurricularAchievementsList
      : FallbackAchievementsData;

      // Urutkan berdasarkan tanggal terbaru
  const sortedExtracurricularAchievementsList = [...source].sort(
    (a, b) => new Date(b.date) - new Date(a.date)
  );

  // Batasi jumlah item berdasarkan maxItems
  const ExtracurricularAchievementsList = sortedExtracurricularAchievementsList.slice(0, maxItems);
  ...
  return (
    ...
    >
      {ExtracurricularAchievementsList.map((item) => {
