...
import FallbackAgenda from "../Data/DataAgenda";
import { normalizeData } from "../../utils/helper";

// fallback sudah normalisasi otomatis
const FallbackAgendaData = normalizeData(FallbackAgenda);

const ComponentsAgenda = ({ agendaList, maxItems = 20, layout = "grid" }) => {
  const source =
    Array.isArray(agendaList) && agendaList.length
      ? agendaList
      : FallbackAgendaData;

  // Urutkan berdasarkan tanggal terbaru
  const sortedAgenda = [...source].sort(
    (a, b) => new Date(b.date) - new Date(a.date)
  );

  const AgendaList = sortedAgenda.slice(0, maxItems);
  ...
  return (
    ...
    >
      {AgendaList.map((item) => {
        ...
        // format tanggal (pasti Date object sekarang)
