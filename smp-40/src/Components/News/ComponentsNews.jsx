...
import FallbackNews from "../Data/DataNews";
import { normalizeData } from "../../utils/helper";

// fallback sudah normalisasi otomatis
const FallbackNewsData = normalizeData(FallbackNews);

const ComponentsNews = ({ newsList, maxItems = 20, layout = "grid" }) => {
  // Pilih data: jika ada props newsList, pakai itu. Jika tidak, pakai fallback
  const source =
    Array.isArray(newsList) && newsList.length ? newsList : FallbackNewsData;

  // Urutkan berdasarkan tanggal terbaru
  const sortedNews = [...source].sort(
    (a, b) => new Date(b.date) - new Date(a.date)
  );

  // Batasi jumlah item berdasarkan maxItems
  const NewsList = sortedNews.slice(0, maxItems);
